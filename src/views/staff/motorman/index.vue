      ...
      <el-table-column type="expand">
        ...
        <template slot-scope="props" v-if="!isDialog">
          ...
          >
            ...
            <el-table-column sortable prop="date" label="日期" width="100" v-if="!isDialog">
            </el-table-column>
            <el-table-column prop="store" label="店名"> </el-table-column>
            <el-table-column prop="phone" label="联系电话"> </el-table-column>
            <el-table-column prop="address" label="地址" width="300" v-if="!isDialog"> </el-table-column>
            <el-table-column prop="totalAmount" label="进货总数"> </el-table-column>
            <el-table-column prop="totalMoney" label="进货总额"> </el-table-column>
            <el-table-column prop="proceedsMoney" label="收款金额" v-if="!isDialog"> </el-table-column>
            <el-table-column prop="status"  label="配送状态" v-if="!isDialog"></el-table-column>
          </el-table>
        </template>
      </el-table-column>
      <el-table-column prop="name" label="姓名" width="150"> </el-table-column>
      <el-table-column prop="phone" label="联系电话" width="100"></el-table-column>
        <el-table-column prop="grossAmount" label="近一年送件数" min-width="80" v-if="!isDialog"></el-table-column>
          <el-table-column prop="grossAmountMonth" label="当月配送件数" min-width="120" v-if="!isDialog"></el-table-column>
          <el-table-column prop="date" label="入职时间" min-width="200" v-if="!isDialog"></el-table-column>
          <el-table-column prop="pay" label="底薪" min-width="65" v-if="!isDialog"></el-table-column>
          <el-table-column prop="money" label="计件金额" min-width="80"></el-table-column>
          <el-table-column prop="length" label="配送数量" min-width="80" v-if="!isDialog"></el-table-column>
...
<script>
import { pagingList10 ,pagingList20 ,purchaseList} from "@/api/staff/motorman";
export default {
  props:{
    isDialog: Boolean
  },
  ...
  methods: {
    clickCurrentChange(currentRow){
      this.$emit('option-motorman', currentRow)
    },
    // 深度复制
    　clone(jsonObj) {　 
　　    var　buf;　 
...
       let da =[year,month,date].join('-');
    ...
    setList(){
      ...
      this.list.map(item=>{
        ...
        item.data.map(item=>{
          item.status= item.status?"已配送":"未配送",
           item.userId=id
           let orderId=item.id
