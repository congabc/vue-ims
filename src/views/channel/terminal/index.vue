<template>
  <div>
    ...
    >
      <el-table-column type="expand">
        ...
        <template slot-scope="props">
          ...
          >
            ...
            <el-table-column label="操作" width="180">
              <template slot-scope="scope">
                  <el-button  type="primary"  size="mini"  @click="handleEdit(scope.$index, scope.row)" >添加品项</el-button>
                <el-button
                  size="mini"
                  type="danger"
                  @click="childrenHandleDelete(scope.$index, scope.row)"
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <el-button  type="primary"  size="mini"  @click="1(scope.$index, scope.row)" >添加订单</el-button>
    ...
    childrenHandleDelete(index,row){
      this.list.filter(item=>{
        ...
        if(item.id == row.userId){
          ...
          if(row.orderId){
            ...
            item.data.filter(item=>{
              if(item.id==row.orderId){
                ...
                item.data.splice(index,1)
                this.symbolKey = Symbol(new Date().toString())
                item.totalAmount=item.totalAmount-row.amount
                item.totalMoney-=row.money
                item.proceedsMoney =(item.proceedsMoney-0)- (row.money-0)
              }
            })
            console.log(item)
            item.gross-=row.amount
          item.money-=(row.money)
          item.payment-=(row.money-0)
          ...
          // 修改主表格数据
          item.data.splice(index,1)
          item.gross-=row.totalAmount
          item.money-=(row.totalMoney)
          item.payment-=(row.proceedsMoney-0)
          item.arrearage =(item.money-0)- (item.payment-0)
          }
