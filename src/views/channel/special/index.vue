  ...
  <el-table
    ...
    <el-table-column label="配送信息">
      ...
      <el-table-column label="地址">
        ...
        <el-table-column prop="gross" label="出库总量" min-width="65"></el-table-column>
        <el-table-column prop="money" label="应收金额" min-width="100"></el-table-column>
...
<script>
import { pagingList10, pagingList20, pagingList50 } from "@/api/terminal";
export default {
  data() {
    ...
    return {
      ...
      rules: {
        store: [{ required: true, message: "请输入店名", trigger: "blur" }],
        name: [
  ...
  methods: {
    ...
    fetchData(total) {
      ...
      if (this.pageSize == 10) {
        // 接口
        pagingList10(this.currentPage, this.pageSize, this.formInline).then(
          (response) => {
            getlist(response);
          }
        );
      } else if (this.pageSize == 20) {
        pagingList20(this.currentPage, this.pageSize, this.formInline).then(
          (response) => {
            getlist(response);
          }
        );
      } else {
        pagingList50(this.currentPage, this.pageSize, this.formInline).then(
          (response) => {
            getlist(response);
          }
        );
